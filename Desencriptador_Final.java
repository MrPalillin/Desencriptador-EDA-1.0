...
public class Desencriptador4 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			DataInputStream ec = new DataInputStream(sc); 				// Necesitamos leer los bits del archivo sin signo,de lo contrario daría una excepción por leer una posición negativa en un array
			...
			for (int i = 0; i < 65535; i++) {
				...
				if ((pos = buscar(datos, cadena)) != -1) { 				// Devuelve -1 cuando termina el mensaje
					System.out.println(i); 								// Clave con la que se descifra el mensaje
					System.out.println(vec2str(datos, pos - 100, pos + 500, longitud));
				}
			}
	...
	public static boolean comparacion(short[] texto, short[] clave) { 	// Devuelve false si no coincide la cadena buscada,y true si coincide
	...
	public static int buscar(short[] texto, String cadena) { 			// Busca la secuencia de carácteres de la cadena buscada en el texto
		char[] busqueda = cadena.toCharArray();
		short[] clave = new short[busqueda.length];
		short[] comparador = new short[clave.length];
		for (int cont = 0; cont < clave.length; cont++) {
			clave[cont] = (short) busqueda[cont];
		}

		for (int i = 0; i < (texto.length - clave.length); i++) {
			for (int j = 0; (j + i) < (i + clave.length); j++) {
				comparador[j] = texto[j + i];
			}

			if (comparacion(comparador, clave)) {
				return i;
			}
		}
		return -1;
	...
	public static void ofuscar(short[] datos, int clave, int longitud) { // Algoritmo de desofuscación asignado
		int w0, w1, b;
